...
def create_mask(
    ...
    tgt: TensorType["BATCH_SIZE", "src_len", long],
    pad_idx: int,
) -> tuple[
    ...
    padding_mask_src: TensorType["BATCH_SIZE", "src_len", bool] = src == pad_idx
    padding_mask_tgt: TensorType["BATCH_SIZE", "tgt_len", bool] = tgt == pad_idx
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model: int, max_len: int, dropout: float = 0.1) -> None:
        ...
        positional_encoding: TensorType[
            1, max_len, d_model, float32
        ] = pos_enc.unsqueeze(0)
        self.register_buffer("positional_encoding", positional_encoding)

    def forward(
        self, token_emb: TensorType["batch_size", "token_len", "d_model", float32]
    ) -> TensorType["batch_size", "token_len", "d_model", float32]:
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        src_emb: TensorType[
            "batch_size", "src_len", "d_model", float32
        ] = self.pos_encoder(self.src_embedding(src))

        tgt_emb: TensorType[
            "batch_size", "tgt_len", "d_model", float32
        ...
        output: TensorType[
            "batch_size", "tgt_len", "tgt_vocab_size", float32
